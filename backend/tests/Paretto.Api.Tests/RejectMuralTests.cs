...
/// <summary>
/// Block 4 (Rechazar mural, admin) de FEAT-001c — POST /api/moderation/murals/{id}/reject.
///
/// Misma estrategia de testing que ApproveMuralTests: integración HTTP completa vía
...
/// </summary>
public class RejectMuralTests : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly WebApplicationFactory<Program> _baseFactory;

    public RejectMuralTests(WebApplicationFactory<Program> baseFactory)
    {
    ...
    private sealed class FakeBlobStorageService : IBlobStorageService
    {
        public Task<string> UploadAsync(Stream content, string blobName, string contentType, CancellationToken ct) =>
            throw new NotSupportedException("Not needed for RejectMural tests.");
    ...
    /// `ApproveMuralTests`'s own `ThrowingSaveChangesDbContext`), used to exercise
    /// `RejectMuralCommandHandler`'s `catch (DbUpdateException)` -&gt; `ModerationPersistenceException`
    ...
    private sealed class ThrowingSaveChangesDbContext : AppDbContext
    {
        ...
        public override Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new DbUpdateException("Simulated DB failure while rejecting the mural.");
            }
    ...
    [Fact]
    public async Task Administrator_rejects_a_pending_mural_and_gets_200_with_rejected_status()
    {
        ...
        var response = await client.PostAsync($"/api/moderation/murals/{muralId}/reject", null);
        ...
        Assert.Equal(MuralStatus.Rejected, statusInDb);
    }

    [Fact]
    public async Task Standard_user_attempting_to_reject_gets_403_and_mural_stays_pending()
    {
        var factory = CreateFactory(Guid.NewGuid().ToString());
        var (ownerId, _, _) = await SeedUserAsync(factory);
        var muralId = await SeedMuralAsync(factory, ownerId, MuralStatus.Pending);
    ...
    [Fact]
    public async Task Rejecting_a_nonexistent_mural_returns_404()
    {
        var factory = CreateFactory(Guid.NewGuid().ToString());
        var (_, adminUsername, adminPassword) = await SeedUserAsync(factory, UserRole.Administrator);
        ...
        var response = await client.PostAsync($"/api/moderation/murals/{Guid.NewGuid()}/reject", null);
    ...
    public async Task Rejecting_a_mural_that_is_already_published_or_rejected_returns_409(MuralStatus initialStatus)
    {
        var factory = CreateFactory(Guid.NewGuid().ToString());
        var (ownerId, _, _) = await SeedUserAsync(factory);
