...
- FR-01: El sistema debe permitir crear una cuenta nueva ingresando un nombre de usuario, contraseña
  y email. (RF-040)
- FR-02: El sistema debe rechazar el registro, informando el motivo, cuando el email ya está
  asociado a una cuenta existente. (RF-040)
- FR-03: El sistema debe rechazar el registro, informando el motivo, cuando el nombre de usuario ya
  está en uso. (RF-040)
- FR-04: El sistema debe rechazar el registro, informando el motivo, cuando la contraseña tiene
  menos de 8 caracteres, más de 128 caracteres, o no incluye letras y números. (RF-040, RF-054)
- FR-05: El sistema debe permitir al usuario autenticarse con usuario y contraseña. (RF-022)
- FR-06: El sistema debe rechazar el intento de login cuando el usuario o la contraseña son
  incorrectos, sin indicar cuál de los dos falló.
- FR-07: El sistema debe permitir al usuario cerrar su sesión activa. (RF-033)
- FR-08: El sistema debe almacenar cada cuenta con un campo de rol, con valor por defecto
...
- AC-01: WHEN un visitante completa el registro con un nombre de usuario, contraseña y email no
  ...
  sesión con esas credenciales. (FR-01, FR-08)
- AC-02: IF el email ingresado en el registro ya está asociado a una cuenta existente, THEN THE
  sistema SHALL rechazar el registro e informar que el email ya está en uso. (FR-02)
- AC-03: IF el nombre de usuario ingresado en el registro ya está en uso, THEN THE sistema SHALL
  rechazar el registro e informar que el usuario ya está en uso. (FR-03)
- AC-04: IF la contraseña ingresada en el registro tiene menos de 8 caracteres, más de 128
  caracteres, o no incluye letras y números, THEN THE sistema SHALL rechazar el registro e informar
  el motivo. (FR-04)
- AC-05: WHEN el usuario ingresa su usuario y contraseña correctos, THE sistema SHALL autenticarlo y
  emitir una sesión válida por 7 días. (FR-05)
- AC-06: IF el usuario o la contraseña ingresados son incorrectos, THEN THE sistema SHALL rechazar
  el intento de login con un mensaje genérico, sin indicar cuál de los dos campos falló. (FR-06)
- AC-07: WHEN un usuario con sesión activa cierra sesión, THE sistema SHALL invalidar su sesión,
  exigiendo autenticación nuevamente para acceder a funcionalidades protegidas. (FR-07)
...
- **Gestión de roles vía interfaz.** El campo de rol se modela en el esquema (FR-08), pero asignar
...
**Mitigación:** cierre de sesión explícito disponible (FR-07); sesión servida exclusivamente sobre
...
vector expuesto y se documenta como riesgo conocido, no silencioso.
